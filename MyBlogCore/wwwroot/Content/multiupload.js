		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
 			    ...
 			    if (this.config.imagePath.substr(this.config.imagePath.length - 1, 1) != '/')
 			    ...
 			    //alert(imagepath.substring(imagepath.length-2, 1));

 			    var sampleIcon = '<img src="' + imagepath + 'image.png" />';
				var errorClass = "";
				if(typeof this.items[i] != undefined){
					if(self._validate(this.items[i].type) <= 0) {
					    sampleIcon = '<img src="' + imagepath + 'unknown.png" />';
						errorClass =" invalid";
					} 
					html += '<div class="dfiles' + errorClass + '" rel="' + uId + '"><h5>' + sampleIcon + this.items[i].name + '</h5><div id="' + uId + '" class="progress" style="display:none;"><img src="' + imagepath + 'ajax-loader.gif" /></div></div>';
				}
			}
