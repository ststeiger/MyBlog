...
// https://github.com/mono/aspnetwebstack/tree/master/src/WebMatrix.WebData
...
namespace MyBlog.Trololololol
{
    ...
    internal class DatabaseConnectionInfo
    {
        ...
        private bool bHasConnectionString;
        private bool bHasConnectionStringName;
        ...
        { 
            ...
            if (simpleMembershipProvider != null)
            {
                ...
                Type myType = typeof(WebMatrix.WebData.SimpleMembershipProvider);

                System.Reflection.PropertyInfo prop = myType.GetProperty("ConnectionInfo", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                object objDbConInfo = prop.GetValue(simpleMembershipProvider, null);
                Type tConInfo = objDbConInfo.GetType();

                System.Reflection.PropertyInfo propCSN = tConInfo.GetProperty("ConnectionStringName", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                System.Reflection.PropertyInfo propCS = tConInfo.GetProperty("ConnectionString", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                System.Reflection.PropertyInfo propPN = tConInfo.GetProperty("ProviderName", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);



                ConnectionStringName = (string)propCSN.GetValue(objDbConInfo, null);
                ConnectionString = (string)propCS.GetValue(objDbConInfo, null);
                ProviderName = (string)propPN.GetValue(objDbConInfo, null);

                if (!string.IsNullOrWhiteSpace(ConnectionStringName))
                    bHasConnectionStringName=true;

                if (!string.IsNullOrWhiteSpace(ConnectionString))
                    bHasConnectionString = true;
            }
        }
        ...
        // http://blog.neteril.org/blog/2011/01/22/webmatrix-data-on-mono/
        // public WebMatrix.Data.Database Connect()
        internal virtual IDatabase Connect()
        {
                if(bHasConnectionStringName)
                    return new DatabaseWrapper(
                        Database.Open(ConnectionStringName)
                );
        
                if(bHasConnectionString)
                    return new DatabaseWrapper(
                        Database.OpenConnectionString(ConnectionString, ProviderName)
                );

                return new DatabaseWrapper(null);
                //return new DatabaseWrapper(ConnectionInfo.Connect());

            throw new NullReferenceException("No membership provider that is castable into WebMatrix.WebData.SimpleMembershipProvider");
        }
    ...
    {
        public static string Security_InitializeMustBeCalledFirst = "Initialize must be called first";
        public static string SimpleMembership_ProviderUnrecognizedAttribute = "Provider unrecognized";
        public static string SimpleMembership_FailToStoreOAuthToken = "Failed to store OAuth token";
        public static string SimpleMembership_PasswordTooLong = "Password too long";
        public static string Security_NoUserFound = "No user found";
        public static string Security_DbFailure = "Db Failure";
        public static string Security_NoAccountFound = "No account found";
        public static string Security_FailedToFindUserTable = "Failed to find user table";    
    }
    ...
    public class MySimpleMembershipProvider : WebMatrix.WebData.SimpleMembershipProvider //: ExtendedMembershipProvider
    {
        private const int TokenSizeInBytes = 16;
        private readonly MembershipProvider _previousProvider;


        public MySimpleMembershipProvider() : this(null)
        {
        }


        public MySimpleMembershipProvider(MembershipProvider previousProvider)
        {
        ...
        internal virtual IDatabase ConnectToDatabase()
        {
            ...
            if (simpleMembershipProvider != null)
            {
                ...
                string ConnectionStringName = (string)propCSN.GetValue(objDbConInfo, null);
                string ConnectionString = (string)propCS.GetValue(objDbConInfo, null);
                string ProviderName = (string)propPN.GetValue(objDbConInfo, null);
        ...
        */
        ...
        public string GetPropertyValue(string strPropertyName)
        {
            ...
            if (simpleMembershipProvider != null)
            {
                ...
                System.Reflection.PropertyInfo prop = myType.GetProperty(strPropertyName, System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                string strPropertyValue = (string)prop.GetValue(simpleMembershipProvider, null);
        ...
        public string UserTableName
        {
            get 
            {
                if(m_UserTableName == null)
                    m_UserTableName = GetPropertyValue("UserTableName");

                return m_UserTableName;
            }
            
            set 
            {
                m_UserTableName = value;
            }
        }


        private string m_UserNameColumn;
        // represents the User created UserName column, i.e. Email
        public string UserNameColumn
        {
            get
            {
                if (m_UserNameColumn == null)
                    m_UserNameColumn = GetPropertyValue("UserNameColumn");

                return m_UserNameColumn;
            }

            set
            {
                m_UserNameColumn = value;
            }
        }
        ...
        public string UserIdColumn
        {
            get
            {
                if (m_UserIdColumn == null)
                    m_UserIdColumn = GetPropertyValue("UserIdColumn");

                return m_UserIdColumn;
            }

            set
            {
                m_UserIdColumn = value;
            }
        }
        ...
        private DatabaseConnectionInfo m_DatabaseConnectionInfo;
        internal DatabaseConnectionInfo ConnectionInfo 
        { 
            get{
                if (m_DatabaseConnectionInfo == null)
                    m_DatabaseConnectionInfo = new DatabaseConnectionInfo();

                return m_DatabaseConnectionInfo;
            }
            set{
                throw new NotSupportedException("Cannot set ConnectionInfo");
        }
        ...
        {
            get{
                return WebMatrix.WebData.WebSecurity.Initialized;
            }
            /*
            set{
        ...
        internal void CreateTablesIfNeeded()
        {
            ...
            using (IDatabase db = ConnectionInfo.Connect())
            {
        ...
        private static void CreateOAuthTokenTableIfNeeded(IDatabase db)
        {
            ...
            } // End if (!CheckTableExists(db, OAuthTokenTableName))
        }
        ...
        public override bool ConfirmAccount(string accountConfirmationToken)
        {
            ...
            using (IDatabase db = ConnectionInfo.Connect())
            {
                ...
                var rows = db.Query("SELECT [UserId], [ConfirmationToken] FROM " + MembershipTableName + " WHERE [ConfirmationToken] = @0", accountConfirmationToken)
                    ...
                    .ToList();
                System.Diagnostics.Debug.Assert(rows.Count < 2, "By virtue of the fact that the ConfirmationToken is random and unique, we can never have two tokens that are identical.");
                if (!rows.Any())
                {
                    return false;
                }
