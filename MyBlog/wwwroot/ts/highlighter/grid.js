var HEX_CRAD = 32, HEX_BG = '#171717', HEX_HL = '#2a2a2a', HEX_HLW = 2, HEX_GAP = 4, NEON_PALETE = [
    '#cb3301', '#ff0066',
    '#ff6666', '#feff99',
    '#ffff67', '#ccff66',
    '#99fe00', '#fe99ff',
...
}), nwp = wp.length, csi = 0, f = 1 / T_SWITCH, c = document.querySelectorAll('canvas'), n = c.length, w, h, _min, ctx = [], grid, source = null, t = 0, request_id = null;
var GridItem = (function () {
    ...
    GridItem.prototype.draw = function (ct) {
        for (var i = 0; i < 6; i++) {
            ct[(i === 0 ? 'move' : 'line') + 'To'](this.points.hex[i].x, this.points.hex[i].y);
        }
    };
    GridItem.prototype.highlight = function (ct) {
        for (var i = 0; i < 3; i++) {
...
var Grid = (function () {
    ...
    Grid.prototype.draw = function (ct) {
        ct.fillStyle = HEX_BG;
        ct.beginPath();
...
function init() {
    var s = getComputedStyle(c[0]), rows, cols;
...
function neon() {
    var k = (t % T_SWITCH) * f, rgb = {
        ...
        'b': ~~(wp[csi].b * (1 - k) +
            wp[(csi + 1) % nwp].b * k)
    }, rgb_str = 'rgb(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ')', light = ctx[0].createRadialGradient(source.x, source.y, 0, source.x, source.y, .875 * _min), stp;
